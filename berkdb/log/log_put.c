...
static int __log_put_int_int(dbenv, lsnp, contextp, udbt, flags, off_context, usr_ptr)
	DB_ENV *dbenv;
	DB_LSN *lsnp;
	u_int64_t *contextp;
	const DBT *udbt;
...
err:
	...
	if (udbt->data == dbt->data) {
		...
		printf("%s - different mem\n", __func__);
	}
	if (udbt->size == dbt->size) {
		printf("%s - same size\n", __func__);
	} else {
		printf("%s - different size\n", __func__);
	}
	if (memcmp(udbt->data, dbt->data, udbt->size) == 0) {
		printf("%s - same buf\n", __func__);
	} else {
		printf("%s - different buf\n", __func__);
	}
